...
export class PokemonService {
	...
	getPokemons(url: string): Observable<Pokemons> {
		return this.httpClient
			.get<Pokemons>(environment.URL_SERVER + 'pokemons?url=' + url)
	...
	getPokemonById(id: number): Observable<SpecificPokemon> {
		return this.httpClient
			.get<SpecificPokemon>(environment.URL_SERVER + 'pokemons/id?pokemon=' + id)
			.pipe(retry(2), catchError(this.handleError));
	}

	getPokemonsByType(type: string): Observable<Pokemon[]> {
		return this.httpClient
			.get<Pokemon[]>(environment.URL_SERVER + 'pokemons/bytype?type=' + type)
			.pipe(retry(2), catchError(this.handleError));
	}

	getPokemonsByFilter(search: string): Observable<Pokemon[]> {
		return this.httpClient
			.get<Pokemon[]>(environment.URL_SERVER + 'pokemons/filter?search=' + search)
			.pipe(retry(2), catchError(this.handleError));
	}
